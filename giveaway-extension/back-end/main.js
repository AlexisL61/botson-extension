...
module.exports = {
    start(){
        ...
        if (!fs.existsSync(dataFolder+'/bot-data/giveaways.json')){
            fs.writeFileSync(dataFolder+'/bot-data/giveaways.json',JSON.stringify([]))
        }
        var giveawayTable = JSON.parse(fs.readFileSync(dataFolder+"/bot-data/giveaways.json"))
        ...
        ipcRenderer.on("giveaway-extension-create-giveaway", async function(event,data){
            if (!giveawayAlreadyCreate.find(ga=>ga == data.id)){
                ...
                if (client.channels.cache.has(data.channel)){
                    ...
                    data.users = []
                    giveawayTable = JSON.parse(fs.readFileSync(dataFolder+"/bot-data/giveaways.json"))
                    giveawayTable.push(data)
                    fs.writeFileSync(dataFolder+'/bot-data/giveaways.json',JSON.stringify(giveawayTable))
                }
            }
        })

        client.on('messageReactionAdd', async (reaction, userReact) => {
            console.log(giveawayTable)
            console.log("react")
            if (reaction.emoji.name == "🎉" && !userReact.bot){
                console.log("name")
                giveawayTable = JSON.parse(fs.readFileSync(dataFolder+"/bot-data/giveaways.json"))
                if (giveawayTable.find(ga=>ga.message == reaction.message.id)){
                    console.log("find")
                    var thisGiveaway = giveawayTable.find(ga=>ga.message == reaction.message.id)
        ...
        function start(){
            var totalGiveaway = giveawayTable = JSON.parse(fs.readFileSync(dataFolder+"/bot-data/giveaways.json"))
            for (var i in totalGiveaway){
                var thisGiveaway = totalGiveaway[i]
                if (client.channels.cache.has(thisGiveaway.channel)){
                    var thisChannel = client.channels.cache.get(thisGiveaway.channel)
                    thisChannel.guild.members.fetch()
                    thisChannel.messages.fetch(thisGiveaway.message,true)
                }
            }
        }
        start()
